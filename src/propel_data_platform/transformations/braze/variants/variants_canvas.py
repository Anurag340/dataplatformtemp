...
class BrazeVariantsCanvasTransform(BrazeBaseTransform):
    ...
    def row_transform(row):
        return CanvasUtils.extract_variants(row["stats"], row["canvas_id"])
    ...
    def _write_to_db(self):
        df = self.spark.sql(
            ...
            lambda row: BrazeVariantsCanvasTransform.row_transform(row.asDict())
        )
        df = (
