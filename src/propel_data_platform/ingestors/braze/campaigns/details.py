...
from propel_data_platform.connectors.braze import BrazeCampaignsDetailsConnector
from propel_data_platform.ingestors.braze.base import BaseBrazeIngestor


class BrazeCampaignsDetailsIngestor(BaseBrazeIngestor):

    ENDPOINT = BrazeCampaignsDetailsConnector.ENDPOINT
    ...
    def run(self):
        ...
        self.spark.sql(
            ...
            WITH ranked_rows AS (
                SELECT 
                    *,
                    ROW_NUMBER() OVER (PARTITION BY campaign_id ORDER BY created_at DESC) AS row_rank
