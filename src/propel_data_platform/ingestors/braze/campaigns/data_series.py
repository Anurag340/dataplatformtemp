...
from propel_data_platform.connectors.braze import BrazeCampaignsDataSeriesConnector
from propel_data_platform.ingestors.braze.base import BaseBrazeIngestor


class BrazeCampaignsDataSeriesIngestor(BaseBrazeIngestor):

    ENDPOINT = BrazeCampaignsDataSeriesConnector.ENDPOINT
    ...
    def run(self):
        ...
        def parse_json_line(line):
            try:
                ...
                for data in row.get("data", []):
                    processed_row = {
                        "campaign_id": str(row["metadata"]["params"]["campaign_id"]),
        ...
        self.spark.sql(
            ...
            WITH ranked_rows AS (
                SELECT 
                    *,
                    ROW_NUMBER() OVER (PARTITION BY campaign_id, start_date ORDER BY created_at DESC) AS row_rank
