...
from propel_data_platform.connectors.braze import BrazeCanvasDetailsConnector
from propel_data_platform.ingestors.braze.base import BaseBrazeIngestor


class BrazeCanvasDetailsIngestor(BaseBrazeIngestor):

    ENDPOINT = BrazeCanvasDetailsConnector.ENDPOINT
    ...
    def run(self):
        ...
        self.spark.sql(
            ...
            WITH ranked_rows AS (
                SELECT 
                    *,
                    ROW_NUMBER() OVER (PARTITION BY canvas_id ORDER BY created_at DESC) AS row_rank
