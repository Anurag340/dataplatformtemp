...
class CustomerioOutputsIngestor(BaseCustomerioIngestor):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.destination_table = kwargs.get("config", {}).get("destination-table")
        self.container_prefix += "/outputs"

    def run(self):
        self._update_parquet_metadata(
            file_type="outputs", time_cols=["created_at", "delay_ends_at"]
        )
        ...
        results = self.spark.createDataFrame(results, schema=target_outputs_schema)
