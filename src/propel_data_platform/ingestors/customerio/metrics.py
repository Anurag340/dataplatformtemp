...
target_metrics_schema = StructType(
    [
...
class CustomerioMetricsIngestor(BaseCustomerioIngestor):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.destination_table = kwargs.get("config", {}).get("destination-table")
        self.container_prefix += "/metrics"

    def run(self):
        self._update_parquet_metadata(file_type="metrics", time_cols=["created_at"])
        ...
        results = self.spark.createDataFrame(results, schema=target_metrics_schema)
        ...
        self.spark.sql(
            ...
            WITH ranked_rows AS (
                SELECT 
                    *,
                    ROW_NUMBER() OVER (PARTITION BY event_id ORDER BY inserted_at DESC) AS row_rank
