...
class BrazeCampaignsListConnector(BrazeBaseConnector):
    """Connector for fetching Braze campaigns list data.
    ...
    def fetch(
        ...
        if last_edit_time is not None:
            params["last_edit.time[gt]"] = last_edit_time
        ...
        try:
            ...
            if not response_json.get("message") == "success":
                raise Exception(f"Failed to fetch campaigns")
    ...
    def fetch_all(self):
        ...
        while True:
            ...
            if not success:
                ...
                continue
            page_campaigns = response.get("campaigns", [])
            if len(page_campaigns) == 0:
                break
